...
1. プリセット管理ダイアログの `key: value` グリッド化
2. `CTkTabview` を使った設定ダイアログのタブ化
3. `customtkinter` テーマ JSON への配色ルール集約
...
- [ ] 中央の情報ダンプを `key: value` の整列表現へ置き換える
- [ ] `種別 / ID / サイズ / 形式 / 品質 / EXIF / GPS / ドライラン / 更新日時` を個別行に分解する
- [ ] 情報表示とアクションボタンの距離感を整理する
