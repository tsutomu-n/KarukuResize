...
class App(ctk.CTk):
    def __init__(self):
        ...
        try:
            if (
                hasattr(self.tab_view, "_segmented_button")
                and self.tab_view._segmented_button is not None
            ):
                ...
                print(
                    "Debug: _segmented_button not found or is None, cannot set tab font directly."
                )
        ...
        self.tab_batch_process = self.tab_view.add(
            "一括処理"
        )  # This tab is for batch processing
    ...
    def create_tab_content_frames(self):
        ...
        self.batch_process_content_frame.pack(
        ...
        self.label_batch_input_folder.grid(
        ...
        self.button_browse_batch_input = ctk.CTkButton(
            self.batch_process_content_frame,
            text="選択...",
            font=self.button_font,
            width=80,
        ...
        self.button_browse_batch_input.grid(
            row=0, column=2, padx=(5, 0), pady=5, sticky="e"
        )
        ...
        self.label_batch_output_folder.grid(
        ...
        self.entry_batch_output_folder.grid(
            row=1, column=1, padx=5, pady=5, sticky="ew"
        )

        self.button_browse_batch_output = ctk.CTkButton(
            self.batch_process_content_frame,
            text="選択...",
            font=self.button_font,
            width=80,
        ...
        self.button_browse_batch_output.grid(
            row=1, column=2, padx=(5, 0), pady=5, sticky="e"
        )

        # --- リサイズ設定 ---
        self.label_batch_resize_settings = ctk.CTkLabel(
            self.batch_process_content_frame,
            text="リサイズ設定",
            font=self.heading_font,
        )
        self.label_batch_resize_settings.grid(
            row=2, column=0, columnspan=3, pady=(10, 5), sticky="w"
        )

        # リサイズモード
        self.label_batch_resize_mode = ctk.CTkLabel(
            self.batch_process_content_frame, text="モード:", font=self.normal_font
        )
        self.label_batch_resize_mode.grid(
            row=3, column=0, padx=(0, 5), pady=5, sticky="w"
        )

        self.batch_resize_mode_var = ctk.StringVar(value="指定なし")
        self.batch_resize_modes = [
            "指定なし",
            "幅を指定",
            "高さを指定",
            "縦横最大",
        ...
        # リラジオボタンの再配置調整 (手動)
        # "指定なし" (0)
        self.batch_radio_buttons_resize_mode[0].grid(
            row=4, column=0, padx=2, pady=2, sticky="w"
        )
        # "幅を指定" (1)
        self.batch_radio_buttons_resize_mode[1].grid(
            row=4, column=1, padx=2, pady=2, sticky="w"
        )
        # "高さを指定" (2)
        self.batch_radio_buttons_resize_mode[2].grid(
            row=4, column=2, padx=2, pady=2, sticky="w"
        )
        # "縦横最大" (3)
        self.batch_radio_buttons_resize_mode[3].grid(
            row=5, column=0, padx=2, pady=2, sticky="w"
        )
        # "パーセント指定" (4)
        self.batch_radio_buttons_resize_mode[4].grid(
            row=5, column=1, padx=2, pady=2, sticky="w"
        )

        # リサイズ値
        self.label_batch_resize_value = ctk.CTkLabel(
            self.batch_process_content_frame, text="値:", font=self.normal_font
        )
        self.label_batch_resize_value.grid(
            row=6, column=0, padx=(0, 5), pady=5, sticky="w"
        )
        ...
        self.batch_resize_value_unit_label = ctk.CTkLabel(
            self.batch_process_content_frame, text="px", font=self.normal_font
        )
        self.batch_resize_value_unit_label.grid(
            row=6, column=2, padx=(0, 5), pady=5, sticky="w"
        )
        ...
        self.checkbox_batch_keep_aspect_ratio.grid(
            row=7, column=0, columnspan=3, padx=5, pady=10, sticky="w"
        )
        ...
        self.batch_separator2 = ctk.CTkFrame(
            self.batch_process_content_frame, fg_color="#AAAAAA", height=1
        )
        ...
        self.label_batch_compress_settings.grid(
            row=9, column=0, columnspan=3, pady=(0, 5), sticky="w"
        )

        # 圧縮を有効にするか
        self.batch_enable_compression_var = ctk.BooleanVar(
            value=False
        )  # デフォルトは無効
        ...
        self.checkbox_batch_enable_compression.grid(
        ...
        self.label_batch_output_format.grid(
            row=11, column=0, padx=(0, 5), pady=5, sticky="w"
        )
        ...
        self.optionmenu_batch_output_format.grid(
            row=11, column=1, columnspan=2, padx=5, pady=5, sticky="ew"
        )
        ...
        self.slider_batch_jpeg_quality = ctk.CTkSlider(
            ...
            command=lambda x: self.label_batch_jpeg_quality_value.configure(
                text=f"{int(x)}"
            ),
        )
        ...
        self.slider_batch_webp_quality = ctk.CTkSlider(
            ...
            command=lambda x: self.label_batch_webp_quality_value.configure(
                text=f"{int(x)}"
            ),
        )
        ...
        self.batch_separator3.grid(
            row=15, column=0, columnspan=3, sticky="ew", pady=10
        )  # rowは適宜調整
        ...
        self.label_batch_other_settings.grid(
            row=16, column=0, columnspan=3, pady=(0, 5), sticky="w"
        )  # rowは適宜調整

        # EXIF情報
        self.label_batch_exif = ctk.CTkLabel(
            self.batch_process_content_frame, text="EXIF情報:", font=self.normal_font
        )
        self.label_batch_exif.grid(
            row=17, column=0, padx=(0, 5), pady=5, sticky="w"
        )  # rowは適宜調整
        ...
        self.optionmenu_batch_exif_handling.grid(
            row=17, column=1, columnspan=2, padx=5, pady=5, sticky="ew"
        )  # rowは適宜調整
        ...
        self.label_batch_prefix.grid(
            row=18, column=0, padx=(0, 5), pady=5, sticky="w"
        )  # rowは適宜調整
        ...
        self.entry_batch_prefix.grid(
            row=18, column=1, columnspan=2, padx=5, pady=5, sticky="ew"
        )  # rowは適宜調整
        ...
        self.label_batch_suffix.grid(
            row=19, column=0, padx=(0, 5), pady=5, sticky="w"
        )  # rowは適宜調整
        ...
        self.checkbox_batch_process_subfolders.grid(
            row=20, column=0, columnspan=3, padx=5, pady=10, sticky="w"
        )  # rowは適宜調整

    def browse_batch_input_folder(self):
        folder_selected = filedialog.askdirectory(
            title="一括処理する入力フォルダを選択"
        )
        if folder_selected:
    ...
    def update_batch_quality_settings_visibility(
        self, _event=None
    ):  # _eventはOptionMenuのcommandから渡されるため追加
        # 一旦すべての品質設定UIを非表示にする
        ...
        if selected_format == "JPEG":
            self.label_batch_jpeg_quality.grid(
                row=current_row, column=0, padx=(0, 5), pady=5, sticky="w"
            )
            self.slider_batch_jpeg_quality.grid(
                row=current_row, column=1, padx=5, pady=5, sticky="ew"
            )
            self.label_batch_jpeg_quality_value.grid(
                row=current_row, column=2, padx=(0, 5), pady=5, sticky="w"
            )
            self.slider_batch_jpeg_quality.configure(state="normal")
        elif selected_format == "WEBP":
            self.label_batch_webp_quality.grid(
                row=current_row, column=0, padx=(0, 5), pady=5, sticky="w"
            )
            self.slider_batch_webp_quality.grid(
                row=current_row, column=1, padx=5, pady=5, sticky="ew"
            )
            self.label_batch_webp_quality_value.grid(
                row=current_row, column=2, padx=(0, 5), pady=5, sticky="w"
            )
            current_row += 1
            self.checkbox_batch_webp_lossless.grid(
                row=current_row, column=1, columnspan=2, padx=5, pady=5, sticky="w"
            )
    ...
    def start_resize_process(self):
        ...
        if not input_file_str or not output_dir_str:
            ...
            self.finish_resize_process(
                success=False, message="入力または出力先が未指定"
            )
            return
        ...
        if not output_directory.is_dir():
            try:
                ...
                self.add_log_message(
                    f"出力ディレクトリを作成しました: {output_directory}"
                )
                ...
                self.finish_resize_process(
                    success=False, message="出力ディレクトリ作成失敗"
                )
                return
        ...
        try:
            ...
            self.add_log_message(
                f"エラー: 品質値 '{quality}'は不正な数値です。", is_error=True
            )
        ...
        try:
            ...
            self.add_log_message(
                f"画像処理の開始中に予期せぬエラーが発生しました: {e}", is_error=True
            )
    ...
    def request_cancel_processing(self):
        ...
        self.add_log_message(
            "中断リクエストを受け付けました。現在の処理ステップが完了次第、停止します。"
        )
        ...
        try:
            ...
            if self.cancel_requested:
                self.after(
                    ...
                    lambda: self.add_log_message(
                        "処理が中断されました (スレッド開始直後)。", is_warning=True
                    ),
                )
                self.after(
                    0,
                    lambda: self.finish_resize_process(
                        success=False, message="処理がユーザーによって中断されました。"
                    ),
                )
                return
            ...
            try:
                ...
                self.after(
                    0,
                    lambda: self.finish_resize_process(
                        success=False, message="入力ファイルが見つかりません。"
                    ),
                )
                return
            except Exception as e:
                ...
                self.after(
                    0,
                    lambda e=e: self.finish_resize_process(
                        success=False, message="画像ファイルを開けません。"
                    ),
                )
                return
            ...
            if self.cancel_requested:
                self.after(
                    0,
                    lambda: self.add_log_message(
                        "処理が中断されました (画像読み込み後)。", is_warning=True
                    ),
                )
                self.after(
                    0,
            ...
            if core_resize_mode == "width":
                ...
                if keep_aspect_ratio:
                    calculated_target_width = int(
                        original_width * (resize_value / original_height)
                    )
                else:
            ...
            if calculated_target_width <= 0:
                ...
                self.after(
                    0,
                    lambda: self.finish_resize_process(
                        success=False, message="目標幅の計算結果が無効です。"
                    ),
                )
                return
            ...
            if self.cancel_requested:  # Check before core processing
                self.after(
                    0,
                    lambda: self.add_log_message(
                        "処理が中断されました (コア処理開始前)。", is_warning=True
                    ),
                )
                self.after(
                    0,
            ...
            if self.cancel_requested:
                self.after(
                    0,
                    lambda: self.add_log_message(
                        "処理が中断されました (コア処理後)。", is_warning=True
                    ),
                )
                self.after(
                    0,
            ...
            if success:
                if skipped:
                    ...
                    self.after(
                        0,
                        lambda: self.add_log_message(
                            f"画像処理成功: {dest_path.name} (サイズ: {new_size_kb} KB)"
                        ),
                    )
                self.after(
                    0,
                    lambda: self.finish_resize_process(
                        success=True, message="画像処理が正常に完了しました。"
                    ),
                )
            else:
                self.after(
                    0,
                    lambda: self.add_log_message(
                        f"画像処理失敗: {dest_path.name}", is_error=True
                    ),
                )
                self.after(
                    0,
            ...
            self.after(
                0,
                lambda e=e: self.finish_resize_process(
                    success=False, message=f"予期せぬエラー: {e}"
                ),
            )
