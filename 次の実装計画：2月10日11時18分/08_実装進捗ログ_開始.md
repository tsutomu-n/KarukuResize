...
## 追加改善（不具合修正）
1. `詳細設定を表示` の再表示位置を固定（ヘッダー直下へ再配置）し、表示されない不具合を修正。
2. 画像を再選択した際に `current_index` を初期化し、2回目以降の選択でプレビュー更新が止まる不具合を修正。
3. 画像読み込み失敗時にプレビュー/情報欄を確実にクリアする処理を追加。
4. 単体保存時は常に最新設定でリサイズ再計算するよう変更し、古いプレビュー設定で保存される不整合を修正。
5. Tk callback例外を常時 `messagebox` + ログに出すようにして、無反応に見える失敗の可視性を向上。

## 追加改善（再点検）
1. ヘルプダイアログのスクロール処理を `bind_all/unbind_all` からウィンドウローカルbindへ変更。
2. ヘルプの「閉じる」ボタンでも確実にイベント解除されるよう修正。
3. リサイズ後タイトル更新で不必要な入力検証が走る経路を除去（警告ダイアログ連発を防止）。
4. 一括保存時のEXIF日時検証をループ外へ移動（警告がファイル数分出る問題を修正）。
5. `build_exe.py` を補強:
   - Windows/Linuxで期待出力パスを自動判定
   - `pillow_avif` を hidden import 指定
   - 追加hooksディレクトリを自動指定
